...
export function generateCancellationConfirmationEmail(
  ...
  const getTicketMessage = () => {
    if (isTrialLesson) {
      ...
      return '<p style="color: #dc3545;"><strong>チケット:</strong> 前日21時以降のキャンセルのため、チケット1回分が消化されます。</p>';
    }
  };
  ...
  const getTicketMessageText = () => {
    if (isTrialLesson) {
      ...
      return 'チケット: 前日21時以降のキャンセルのため、チケット1回分が消化されます。';
    }
  };
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>ご予約のキャンセルはマイページにて操作可能です。</p>
      <p>下記Prealグループレッスン予約サイトから画面最上段「会員ログイン」へ→「詳細・キャンセルはこちら」から操作をお願いします。</p>
      <p>Prealグループレッスン予約サイト：<a href="https://pilates-reservation-app-main.vercel.app/">https://pilates-reservation-app-main.vercel.app/</a></p>
      <p>グループに関するお問い合わせはこちらから：<a href="https://lin.ee/faHGlyM">https://lin.ee/faHGlyM</a></p>
    </div>
  `
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #d4edda; padding: 20px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #155724; margin-top: 0;">予約詳細</h3>
        <p><strong>レッスン名:</strong> ${lessonTitle}</p>
        <p><strong>日時:</strong> ${lessonDate}</p>
        <p><strong>お支払い:</strong> ${isTrialLesson ? '1000円(当日現金支払い)' : 'チケット1枚'}</p>
      </div>
      <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0;">
        <h4 style="color: #856404; margin-top: 0; font-size: 16px;">注意事項</h4>
        <ul style="color: #856404; margin: 0; font-size: 14px; line-height: 1.5;">
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #fff3cd; padding: 20px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #856404; margin-top: 0;">予約詳細</h3>
        <p><strong>レッスン名:</strong> ${lessonTitle}</p>
        <p><strong>日時:</strong> ${lessonDate}</p>
        <p><strong>会場:</strong> ${lessonLocation}</p>
        <p><strong>予約タイプ:</strong> ${reservationType}</p>
...
export function generateWaitingListCancellationEmail(
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
        <h3 style="color: #495057; margin-top: 0;">予約詳細</h3>
        <p><strong>レッスン名:</strong> ${lessonTitle}</p>
        <p><strong>日時:</strong> ${lessonDate}</p>
        <p><strong>会場:</strong> ${lessonLocation}</p>
        <p><strong>予約タイプ:</strong> ${reservationType}</p>
